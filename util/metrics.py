...
class RunningMetrics(object):
    ...
    def __fast_hist(self, label_gt, label_pred):
        ...
        中文讲解: https://blog.csdn.net/weixin_43143670/article/details/104722381
        np.bincount的第一项设计的很巧妙, 对于2类任务, 第一项(设其为向量z)max=3,min=0; 针对该向量的不同元素即0、1、2、3有如下意义:
        zi = 3时, 即 self.num_classes(=2) * label_gt[mask](=1) + label_pred[mask](=1)  说明真实值=1,预测值=1;  对应TP
        zi = 2时, 即 self.num_classes(=2) * label_gt[mask](=1) + label_pred[mask](=0)  说明真实值=1,预测值=0;  对应FN
        zi = 1时, 即 self.num_classes(=2) * label_gt[mask](=0) + label_pred[mask](=1)  说明真实值=0,预测值=1;  对应FP
        zi = 0时, 即 self.num_classes(=2) * label_gt[mask](=0) + label_pred[mask](=0)  说明真实值=0,预测值=0;  对应TN
        np.bincount()函数统计zi的出现次数,顺序是出现0的次数、出现1的次数、出现2的次数、出现3的次数. 因此返回的hist =[[TN,FP],[FN,TP]]
        """
        mask = (label_gt >= 0) & (label_gt < self.num_classes) #保证标记的正确性（标记的每个元素值在[0, n_class）内），标记正确得到的mask是一个全为true的数组。
